...
function openEnemyBrawlerSelector(event, index) {
    ...
    const alreadySelectedNames = selectedEnemyBrawlers
        .map((brawler, slotIndex) => {

            if (slotIndex === index || !brawler) {
                return null;
            }
...
function selectEnemyBrawler(event, slotIndex, brawlerIndex) {
    ...
    const duplicateExists = selectedEnemyBrawlers.some(
        (selectedBrawler, index) => {
